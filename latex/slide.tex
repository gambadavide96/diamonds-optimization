...
\begin{frame}{Introduction}
    ...
    \begin{itemize}
        ...
        \item Investigating how theoretical properties of the objective function (such as Smoothness and Strongly Convexity) have an impact to the results.
...
\begin{frame}{Dataset Overview: The Diamonds Dataset}
    ...
    \begin{itemize}
        \item \textbf{Target Variable ($y$):} \texttt{price} in US dollars (ranging from \$326 to \$18,823).
        \item \textbf{Numerical Features:} 
        \begin{itemize}
            \item \texttt{carat}: Weight (0.2 - 5.01). The most significant predictor.
...
\begin{frame}{$f(w)$ is strictly convex}
    To investigate the convexity of our objective function, we rely on the formal definition of definite matrices based on their quadratic forms: \textit{A symmetric matrix $M$ is Positive Semi-Definite if $v^T M v \ge 0$ for all vectors $v$, and Positive Definite if $v^T M v > 0$ for all non-zero vectors $v \neq 0$.}\\[0.3cm]
